...
export function AIInsightsPanel({ expenses, currency, monthlyBudget }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.error || err.message || "Failed to load insights";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="glass shadow-soft rounded-3xl border border-border p-6">
      ...
      {!insights && !loading && (
        <p className="mt-4 text-sm text-muted-foreground">
          Tap <b>Generate</b> to get a personalized summary, anomaly alerts, and
          saving tips tailored to your spending.
        </p>
      )}
      {loading && (
...
export function AIChatPanel({ expenses, currency }) {
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-2 text-sm text-muted-foreground">
            ...
            <button
              onClick={() =>
                setInput("How much did I spend on food this month?")
              }
